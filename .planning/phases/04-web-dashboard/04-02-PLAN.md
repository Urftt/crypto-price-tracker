...
must_haves:
  truths:
    ...
    - "The price table auto-refreshes every 30 seconds by default; the interval is configurable via ?interval=N query parameter (in seconds)"
  ...
  artifacts:
    ...
    - path: "src/crypto_price_tracker/static/app.js"
      provides: "Fetch logic, configurable auto-refresh timer, row click handler, modal population"
...
<task type="auto">
  <name>Task 1: Build complete frontend with price table, configurable auto-refresh, and coin detail modal</name>
  <files>
    src/crypto_price_tracker/static/index.html
    src/crypto_price_tracker/static/style.css
    src/crypto_price_tracker/static/app.js
...
- **Configurable refresh interval (WEB-02):** On DOMContentLoaded, read the refresh interval from the URL query string: `const interval = parseInt(new URLSearchParams(window.location.search).get('interval')) || 30;` Then call loadPrices() and setInterval(loadPrices, interval * 1000). This defaults to 30 seconds but allows the user to override via `?interval=N` (e.g., `http://localhost:8000/?interval=10` for 10-second refresh).
...
- Escape key also closes modal
  ...
  <verify>
    <automated>cd /home/james-turing/repos/crypto-price-tracker-v2 && test -f src/crypto_price_tracker/static/index.html && test -f src/crypto_price_tracker/static/style.css && test -f src/crypto_price_tracker/static/app.js && grep -q 'modal' src/crypto_price_tracker/static/index.html && grep -q 'api/coin' src/crypto_price_tracker/static/app.js && grep -q 'api/prices' src/crypto_price_tracker/static/app.js && grep -q 'setInterval' src/crypto_price_tracker/static/app.js && grep -q 'URLSearchParams' src/crypto_price_tracker/static/app.js && echo "PASS: all frontend files exist with required patterns including configurable interval"</automated>
  </verify>
  <done>
    ...
    - Auto-refresh defaults to 30 seconds; configurable via ?interval=N query parameter
...
Note: test_static_css_served and test_static_js_served depend on the static files existing on disk (created by Task 1). This is safe because Task 1 runs before Task 2 in sequential execution. Add a brief comment above these two tests: `# Requires static files on disk (created by Task 1 of 04-02-PLAN)`.
  </action>
  <verify>
  ...
  <done>
    ...
    - Static file tests include comment noting on-disk prerequisite
...
6. app.js reads refresh interval from URLSearchParams with 30-second default
7. style.css contains .up (green) and .down (red) classes for 24h change coloring
</verification>
...
- Table auto-refreshes every 30 seconds by default; visiting http://localhost:8000/?interval=10 refreshes every 10 seconds
